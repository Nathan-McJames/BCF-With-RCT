...
class Tree {
  ...
  void grow(NumericMatrix X, int p, int min_nodesize, NumericVector pp_weights) {
    ...
    NumericVector X_col_subset = X_col[node_vector[grow_index].observations & (pp_weights>0)];
  ...
  // Method for changing
  void change(NumericMatrix X, int p, NumericVector pp_weights) {
    ...
    if(change_index!=-1)
    {
      ...
      X_col = X_col[node_vector[change_index].observations & (pp_weights>0)];
  ...
  bool has_empty_nodes(int min_nodesize, NumericVector pp_weights) {
    ...
    for(int i=0; i<num_nodes; i++)
    {
      if(node_vector[i].in_use & node_vector[i].is_terminal)
      {
        if(sum(node_vector[i].observations & (pp_weights>0))<min_nodesize)
        {
          return true;
        } 
      }
...
// [[Rcpp::export]]
List fast_rct_bcf(NumericMatrix X_mu,
                  NumericMatrix X_mu_extra,
                  NumericVector y,
                  NumericVector Z_rct,
                  NumericVector Z_treat,
                  NumericMatrix X_tau,
                  NumericMatrix X_tau_extra,
                  double alpha_mu,
                  double beta_mu,
                  double alpha_mu_rct,
                  double beta_mu_rct,
                  ...
                  NumericVector precision_pp_weights)
{
  ...
  for(int iter = 0; iter < n_iter; iter++)
  {
    ...
    for(int tree_num = 0; tree_num < n_tree_mu; tree_num++)
    {
      ...
      if(choice == "Grow")
      {
        proposal_tree.grow(X_mu, p_mu, min_nodesize, mu_pp_weights);
      }
      ...
      if(choice == "Change")
      {
        proposal_tree.change(X_mu, p_mu, mu_pp_weights);
      ...
      if(!proposal_tree.has_empty_nodes(min_nodesize, mu_pp_weights))
      {
    ...
    for(int tree_num = 0; tree_num < n_tree_mu_rct; tree_num++)
    {
      ...
      if(choice == "Grow")
      {
        proposal_tree.grow(X_mu_extra, p_mu_extra, min_nodesize, mu_extra_pp_weights);
      }
      ...
      if(choice == "Change")
      {
        proposal_tree.change(X_mu_extra, p_mu_extra, mu_extra_pp_weights);
        proposal_tree.change_update(X_mu_extra);
      }
      ...
      if(!proposal_tree.has_empty_nodes(min_nodesize, mu_extra_pp_weights))
      {
    ...
    for(int tree_num = 0; tree_num < n_tree_tau; tree_num++)
    {
      ...
      if(choice == "Grow")
      {
        proposal_tree.grow(X_tau, p_tau, min_nodesize, tau_pp_weights);
      }
      ...
      if(choice == "Change")
      {
        proposal_tree.change(X_tau, p_tau, tau_pp_weights);
      ...
      if(!proposal_tree.has_empty_nodes(min_nodesize, tau_pp_weights))
      {
    ...
    for(int tree_num = 0; tree_num < n_tree_tau_rct; tree_num++)
    {
      ...
      if(choice == "Grow")
      {
        proposal_tree.grow(X_tau_extra, p_tau_extra, min_nodesize, tau_extra_pp_weights);
      }
      ...
      if(choice == "Change")
      {
        proposal_tree.change(X_tau_extra, p_tau_extra, tau_extra_pp_weights);
        proposal_tree.change_update(X_tau_extra);
      }
      ...
      if(!proposal_tree.has_empty_nodes(min_nodesize, tau_extra_pp_weights))
      {
